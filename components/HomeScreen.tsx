...
const HomeScreen: React.FC<HomeScreenProps> = ({ 
  ...
  // Online profiles with real Firebase photos for automatic slideshow
  const [onlineProfiles, setOnlineProfiles] = useState<Array<{ id: string; name: string; photoUrl: string; profession?: string; profileType?: string }>>([]);
  const [activeOnlineIndex, setActiveOnlineIndex] = useState(0);

  useEffect(() => {
    const unsubscribe = databaseService.subscribeToOnlineProfilesWithPhotos((profiles) => {
  ...
  useEffect(() => {
    if (onlineProfiles.length <= 1) return;
    const interval = setInterval(() => {
      setActiveOnlineIndex((prev) => (prev + 1) % onlineProfiles.length);
    }, 2800);
    return () => clearInterval(interval);
  }, [onlineProfiles.length]);

  const currentOnlineProfile = onlineProfiles.length > 0 ? onlineProfiles[activeOnlineIndex % onlineProfiles.length] : null;
      ...
      <div className="relative z-10 flex flex-col w-full">
        ...
        <main className="w-full p-4 flex flex-col gap-0 pb-12">
            ...
            <div className="flex flex-col gap-4">
                {/* --- BLOC SERVICE EN LIGNE (Cliquable n'importe où pour ouvrir les services en ligne) --- */}
                <div 
                    id="services-en-ligne-card"
                    onClick={() => {
                    ...
                    className="w-full bg-white text-slate-900 rounded-3xl p-4 sm:p-5 shadow-xl border-2 border-emerald-500 relative overflow-hidden transition-all duration-300 cursor-pointer hover:shadow-2xl active:scale-[0.99] group"
                >
                    <div className="absolute top-0 left-0 right-0 h-1.5 bg-gradient-to-r from-emerald-500 via-orange-500 to-blue-500"></div>

                    <div className="flex items-center justify-between gap-3 sm:gap-5">
                        {/* Partie Gauche : Icône + Texte "Recherche des services déjà en ligne" + Bouton */}
                        <div className="flex-1 min-w-0 flex flex-col justify-between space-y-3 sm:space-y-3.5">
                            {/* Titre & Sous-titre */}
                            <div className="flex items-start space-x-3 text-left">
                                <div className="w-11 h-11 sm:w-13 sm:h-13 rounded-2xl bg-gradient-to-br from-orange-500 to-amber-600 flex items-center justify-center text-white shadow-md shrink-0 group-hover:scale-105 transition-transform duration-300">
                                    <ProfileIcon className="w-6 h-6 sm:w-7 sm:h-7" />
                                </div>
                                <div className="flex flex-col min-w-0">
                                    <div className="flex items-center flex-wrap gap-1.5 sm:gap-2">
                                        <span className="text-xs sm:text-sm md:text-base font-black uppercase tracking-tight text-slate-900 leading-tight">
                                            Recherche des services déjà en ligne
                                        </span>
                                        <span className="px-2 py-0.5 rounded-full text-[8px] sm:text-[9px] font-black uppercase bg-emerald-100 text-emerald-800 inline-flex items-center shrink-0">
                                            ACCÈS UNIQUE
                                        </span>
                                    </div>
                                    <p className="text-[10px] sm:text-xs font-bold text-slate-500 mt-0.5">
                                        Point d'accès unique à toutes les recherches
                                    </p>
                                </div>
                            </div>

                            {/* Bouton Services en ligne avec icône œil 👁️ */}
                            <div>
                                <div
                                    className="inline-flex items-center justify-center space-x-2 bg-pink-600 group-hover:bg-pink-700 active:scale-95 text-white rounded-2xl px-4 py-2.5 sm:py-3 shadow-md transition-all font-black text-xs uppercase tracking-wider"
                                    id="services-en-ligne-lightweight"
                                >
                                    {isLoadingServices ? (
                                        <svg className="animate-spin h-4 w-4 sm:h-5 sm:w-5 text-white shrink-0" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                            <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                                            <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                                        </svg>
                                    ) : (
                                        <Search className="w-4 h-4 sm:w-5 sm:h-5 text-white group-hover:scale-110 transition-transform duration-200 shrink-0" />
                                    )}
                                    <span className="text-[11px] sm:text-xs font-black uppercase tracking-wider text-white">
                                        Services en ligne
                                    </span>
                                    <Eye className="w-4 h-4 sm:w-5 sm:h-5 text-white animate-eye-blink shrink-0" />
                                </div>
                            </div>
                        </div>

                        {/* Partie Droite : Grande photo bien visible défilant automatiquement */}
                        <div className="shrink-0">
                            <div 
                                className="relative w-20 h-20 sm:w-24 sm:h-24 md:w-28 md:h-28 rounded-2xl sm:rounded-3xl overflow-hidden shadow-lg border-2 sm:border-3 border-emerald-500 bg-slate-100 shrink-0 flex items-center justify-center group-hover:scale-105 transition-transform duration-300"
                                title={currentOnlineProfile ? `${currentOnlineProfile.name} (${currentOnlineProfile.profession || 'Disponible'}) - En ligne` : "Prestataires en ligne"}
                            >
                                {/* Pastille verte animée indiquant l'état en direct */}
                                <div className="absolute top-1.5 right-1.5 w-3 h-3 sm:w-3.5 sm:h-3.5 bg-emerald-500 rounded-full border-2 border-white shadow-md animate-pulse z-20" />
                                
                                <AnimatePresence mode="wait">
                                    {currentOnlineProfile?.photoUrl ? (
                                        <motion.div
                                            key={currentOnlineProfile.photoUrl + (currentOnlineProfile.id || '')}
                                        ...
                                        >
                                            <img
                                                src={currentOnlineProfile.photoUrl}
                                                alt={currentOnlineProfile.name}
                                                className="w-full h-full object-cover"
                                                referrerPolicy="no-referrer"
                                            />
                                            {/* Légende discrète avec le nom / statut */}
                                            <div className="absolute bottom-0 inset-x-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent p-1 sm:p-1.5 text-center">
                                                <span className="text-[8px] sm:text-[9px] font-black uppercase text-white truncate block tracking-tighter">
                                                    {currentOnlineProfile.name.split(' ')[0]} • En ligne
                                                </span>
                                            </div>
                                        </motion.div>
                                    ) : (
                                        <motion.div
                                            ...
                                            animate={{ opacity: 1 }}
                                            exit={{ opacity: 0 }}
                                            className="w-full h-full flex flex-col items-center justify-center bg-gradient-to-br from-emerald-500 to-teal-700 text-white p-1 text-center"
                                        >
                                            <Eye className="w-6 h-6 sm:w-8 sm:h-8 text-white animate-pulse" />
                                            <span className="text-[8px] sm:text-[9px] font-black uppercase tracking-tight leading-none mt-1">En ligne</span>
                                        </motion.div>
                                    )}
                                </AnimatePresence>
                            </div>
